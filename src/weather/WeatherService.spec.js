...
describe("WeatherService", () => {
    ...
    it("uses the whole day if time now is in the first block", () => {
        ...
        expect(outcome.forecast[0].temperature).toBe(
            mockData.mocks.today[0].temperature
        )
        ...
        expect(outcome.forecast[7].temperature).toBe(
            mockData.mocks.today[7].temperature
        )
    ...
    it("spreads into tomorrow if the time now is later in the day", () => {
        ...
        expect(outcome.forecast[7].temperature).toBe(
            mockData.mocks.tomorrow[3].temperature
        )
    ...
    it("warns about sunscreen when it should", () => {
        ...
        const outcome3 = service3.forecast(startOfDay)
...
const mockForWarnings = function (metric, value, mockTime) {
    ...
    for (
        let index = 0;
        index < mockData.data.SiteRep.DV.Location.Period[0].Rep.length;
        index++
    ) {
    ...
    for (
        let index = 0;
        index < mockData.data.SiteRep.DV.Location.Period[1].Rep.length;
        index++
    ) {
